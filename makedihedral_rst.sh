...
usage() {
    echo "Usage: $0 -i <protein.pdb> -r <1,2,3,4,5> -o <output.dat> [-c <A,B>] [-e <other_restraints.dat>]"
    exit 1
}
...
while getopts "i:r:o:c:e:" opt; do
    case ${opt} in
        i ) PDB_FILE=$OPTARG ;;
        r ) RES_LIST=$OPTARG ;;
        o ) OUT_FILE=$OPTARG ;;
...
while IFS= read -r line; do
    if [[ "$line" == ATOM* ]]; then
        ...
        if [ -n "$TARGET_CHAINS" ]; then
            # If a chain is specified, filter and use chain prefix
            if [[ ! ",$TARGET_CHAINS," == *",$chain_id,"* ]]; then
                continue
            fi
            uniq_key="${chain_id}${res_num}"
        else
            # If no chain is specified, completely ignore chain and use number only
            uniq_key="${res_num}"
        fi
...
IFS=',' read -ra RES_ADDR <<< "$RES_LIST"

if [ -n "$TARGET_CHAINS" ]; then
    IFS=',' read -ra CHAIN_ADDR <<< "$TARGET_CHAINS"
else
    # Runs the loop exactly once without a chain prefix
    CHAIN_ADDR=("")
fi
...

{
    for chain in "${CHAIN_ADDR[@]}"; do
        ...
        for i in "${RES_ADDR[@]}"; do
            ...
            # If no target chain, curr_key becomes just the number, matching uniq_key
            curr_key="${chain}${i}"
            prev_key="${chain}$((i - 1))"
            next_key="${chain}$((i + 1))"
